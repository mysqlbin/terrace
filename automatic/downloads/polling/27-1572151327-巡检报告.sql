...
4 状态巡检项:
4.1 InnoDB层缓冲池状态:
   Innodb_buffer_pool_pages_data : 3783
   Innodb_buffer_pool_pages_dirty : 11
   Innodb_buffer_pool_pages_flushed : 2749
   Innodb_buffer_pool_pages_free : 2614
   Innodb_buffer_pool_pages_total : 6400
   Innodb_buffer_pool_read_ahead : 1280
   Innodb_buffer_pool_read_ahead_evicted : 0
   Innodb_buffer_pool_read_requests : 813708
   Innodb_buffer_pool_reads : 2374
   Innodb_buffer_pool_wait_free : 0
   脏页在缓冲池数据页中的占比为: 0.16999999999999998%
   InnoDB buffer pool 缓冲池命中率: 99.55000000000001%
4.2 并发线程连接数:
   Threads_connected : 7
   Threads_created : 14
   Threads_running : 1
...
4.5 创建内存临时表和磁盘临时表的次数:
   Created_tmp_disk_tables : 969
   Created_tmp_tables : 8595
4.6 InnoDB关键特性double write的使用情况:
   Innodb_dblwr_pages_written : 2231
   Innodb_dblwr_writes : 147
   每次写操作合并page的个数: 15.0
