...
def mysql_query(sql,user=user,passwd=passwd,host=host,port=int(port),dbname=dbname,limitnum=select_limit):
    try:
        conn=pymysql.connect(host=host,user='salt_user',passwd='123456abc',port=int(3306),connect_timeout=5,charset='utf8')
        conn.select_db(dbname)
        cursor = conn.cursor()
        count=cursor.execute(sql)
        # count=cursor.execute('select * from myapp_db_account limit 200')
        index=cursor.description
        col=[]
        #get column name
        for i in index:
...
def get_advice(hosttag, sql, request):
    if advisor_switch!=0:
        ...
        tar_port = tar_port
        tar_username = tar_username
        tar_passwd = tar_passwd
        tar_host = tar_host
        tar_dbname = tar_dbname
        ...
        # print cmd
        status,result_tmp = subprocess.getstatusoutput(cmd)
        #return cmd
...
def get_mysql_data(hosttag,sql,useraccount,request,limitnum):
    ...
    for i in a.db_account_set.all():
        if i.role!='write' and i.role!='admin':
            # find the specified account for the useri
    ...
    # results = mysql_query(sql,'salt_user','123456abc','192.168.1.24',3306,tar_dbname,limitnum)
...
def check_mysql_query(sqltext,user,type='select'):
    ...
    while True:
        ...
        if (is_not(lastletter,';')):
            sqltext = sqltext[:-1]
        else:
            break
    #判断语句中是否已经存在limit，has_limit 为0时说明原来语句中是有limit的
    try:
        has_limit = is_(sqltext.split()[-2].lower(),'limit')
    except Exception as e:
